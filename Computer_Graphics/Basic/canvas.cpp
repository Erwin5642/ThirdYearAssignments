...
{  
   if (primitive == LINESEGMENT) glColor3f(1.0, 1.0, 1.0); // Highlight.
   else glColor3f(0.8, 0.8, 0.8); // No highlight.
   glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
...
void drawScene(void)
{
   ...
   drawLines();
   drawRectangles();
   if ( ((primitive == LINESEGMENT) || (primitive == RECTANGLE)) &&
...
void pickPrimitive(int y)
{
   ...
   else if ( y < (1 - 1*0.1)*height ) primitive = LINESEGMENT;
