...
static float k = 0.0; // Animation parameter.
static float Xangle = 0.0, Yangle = 260.0, Zangle = 0.0; // Angles to rotate scene.
static int animateFlower = 0; // Animated?
static int animationPeriod = 5; // Time interval between frames.
static unsigned int base; // Display lists base index.
static int animateHeart = 0;
static double heartScale = 0.0;
static int x = -10;
static int y = 0;
const string text = "Amo a Maria Luiza!";
...
void drawHeart(int numVertices){
   int i;
   float t;
   glBegin(GL_LINE_LOOP);
      for(i = 0; i < numVertices; ++i)
	  {
         t = 2 * PI * i / numVertices; 
         glVertex3f(16 * pow(sin(t), 3), 13 * cos(t) - 5 * cos(2 * t) - 2 * cos(3 * t) - cos(4 * t), 0.0);
	  }
   glEnd();
}
...
void drawScene(void)
{
   ...
   float angleFirstSegment = (1-t)*90.0 + t*80.0; // Angle of first plant segment interpolating between 60 and 90.
   float angleSecondSegment = (1-t)*-20.0 + t*-20.0; // Angle of second plant segment.
   float angleThirdSegment = (1-t)*-20.0 + t*-20.0; // Angle of third plant segment.
   float angleFourthSegment = (1-t)*-20.0 + t*-20.0; // Angle of fourth plant segment.
   ...
   if(animateHeart){
      ...
      glTranslatef(0, 40, 0);
      drawText();
      glScalef(heartScale, heartScale, 1);
      drawHeart(100);
      glPopMatrix();
      
   }
   
   glRotatef(t * 180 + 180, 0, 1, 0);
...
void keyInput(unsigned char key, int x, int y)
{
   switch(key) 
   {
      case 27:
         ...
         break;
	  case ' ': 
         if (animateFlower) animateFlower = 0;
		 else animateFlower = 1;
         glutPostRedisplay();
		 break;
	  case 127: 
         if (animateFlower) animateFlower = 0;
